...
void MemoryCheckA2(Ctree* tree, int* chstart, int*children, double* const memory_sizes, double numProcessors, io_method_t method){//chstart, children are not modified
   ...
   // cout<<endl;
    ...
    while (!subtreeRoots.empty()) {
        ...
        if (memory_required>memorySizeOnCurrentProcessor) {
            ...
            switch (method) {
                case FIRST_FIT:
                    IO_volume = IOCounter(subtree, subtreeSize+1, spacewghts, ewghts, chstartsub, childrensub, schedule_copy, memorySizeOnCurrentProcessor, false, true, com_freq, &BrokenEdgesID, FIRST_FIT);
                    break;
                case LARGEST_FIT:
                    IO_volume = IOCounter(subtree, subtreeSize+1, spacewghts, ewghts, chstartsub, childrensub, schedule_copy, memorySizeOnCurrentProcessor, false, true, com_freq, &BrokenEdgesID, LARGEST_FIT);
                    break;
                case IMMEDIATELY:
                    Immediately(subtree, subtreeSize+1, spacewghts, ewghts, chstartsub, childrensub, schedule_copy, memorySizeOnCurrentProcessor, com_freq,&BrokenEdgesID);
                    break;
